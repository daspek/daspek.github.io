...
OneDriveApp.prototype = {
    initalize: function () {
        ...
        if (that._internalApp._status !== APP_STATUS_INITIALIZED) {
            ...
            that._internalApp.init(initProperties);
    ...
    executeOpenOperation: function () {
        ...
        internalApp.fileDialog(fileDialogProperties).then(
            ...
            function (fileDialogResponse) {
                ...
                // representing the user's file selection. To get the contents
                // of this bundle, we now have to do a GET request against the
                // sharing bundle.
                var apiProperties = {
                    path: fileDialogResponse.id + "/files",
                    method: HTTP_METHOD_GET,
                    interface_method: method
                };

                internalApp.api(apiProperties).then(
                    // Success callback.
                    ...
                    // Error callback.
                    function (apiError) {
                        that.processError(apiError, ERROR_DESC_OPERATION_API);
                    }
                );
            },
            // Error callback.
    ...
    executeSaveOperation: function () {
        ...
        internalApp.fileDialog(fileDialogProperties).then(
            // Success callback.
            function (fileDialogResponse) {
                var folderId = fileDialogResponse.id;
                    ...
                    case UPLOADTYPE_URL:
                        {
                            ...
                            internalApp.api(apiProperties).then(
                                ...
                                // Error callback.
                                function (apiError) {
                                    that.processErrorCallback(apiError, ERROR_DESC_OPERATION_UPLOAD);
                                }
                            );
                    ...
                    case UPLOADTYPE_FORM:
                        {
                            ...
                            // Upload operation.
                            internalApp.upload(uploadProperties).then(
                                // Success callback.
                                ...
                                function (uploadError) {
                                    that.processErrorCallback(uploadError, ERROR_DESC_OPERATION_UPLOAD);
                                },
                                // Progress callback.
                                function (uploadProgress) {
                                    invokeCallback(progress, uploadProgress.progressPercentage);
                                }
                            );
                ...
                function(apiResponse) {
                    switch (apiResponse[API_PARAM_STATUS_HTTP]) {
                        case API_STATUS_HTTP_ACCEPTED:
                            {
                                invokeCallback(progress, apiResponse[API_PARAM_PERCENT_COMPLETE]);
    ...
    createButtonElement: function () {
        /// <summary>
        /// Create the HTML element for the open / save buttons.
        /// </summary>

        var buttonElement = document.createElement("button"),
            properties = {};

        properties[FILEDIALOG_PARAM_MODE] = this._method === IMETHOD_ONEDRIVE_CREATEBUTTON_OPEN ? FILEDIALOG_PARAM_MODE_OPEN : FILEDIALOG_PARAM_MODE_SAVE;
        properties[UI_PARAM_THEME] = this._options[ONEDRIVE_PARAM_THEME] === ONEDRIVE_PARAM_THEME_BLUE ? UI_SIGNIN_THEME_BLUE : UI_SIGNIN_THEME_WHITE;

        // Build button html.
        var buttonHtml = buildSkyDrivePickerControlInnerHtml(properties, true /* new picker */);
        buttonElement.innerHtml = buttonHtml.innerHtml;
        buttonElement.id = buttonHtml.buttonId;
        buttonElement.title = buttonHtml.buttonTitle;

        // Add button style.
        buildSkyDrivePickerControlStyle(properties, buttonElement);
    ...
    processErrorCallback: function(error, operation) {
        ...
        invokeCallback(this._options[ONEDRIVE_PARAM_ERROR], this.processError(error, operation));
    }
};
...
var APIRequest = function (properties) {
    ...
    var path = properties[API_PARAM_PATH],
        coreapi = properties[API_PARAM_COREAPI];
    request._url = isPathFullUrl(path) ?
        path :
        getApiServiceUrl(coreapi) + (path.charAt(0) === "/" ? path.substring(1) : path);
...
function openPopUp(url) {
    var width = 800, height = 600, top, left;
 ...
 */
function getApiServiceUrl(coreApi) {
    return coreApi ? wl_app[ONEDRIVE_API] : wl_app[WL_APISERVICE_URI];
}
...
wl_app[API_X_HTTP_LIVE_LIBRARY] = "Web/DEVICE_" + trimVersionBuildNumber("5.5.8207.2003");
